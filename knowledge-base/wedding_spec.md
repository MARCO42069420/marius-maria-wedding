...
## Guest Photo Gallery (built 2026-07-28, redesigned 2026-07-29 for scale)
...
| **Feedback mode** | Real per-file confirmation via a separate `doGet` verification check (not the opaque `no-cors` POST response) — ✓/✕ badge per thumbnail, auto-retries twice on failure |
| **Concurrency handling** | One file in flight per device at a time, to stay under Apps Script's 30-simultaneous-execution cap when many guests upload at once (~50 expected) |
